...
<xsl:stylesheet 
	...
	<xsl:template match="classDetails">
		...
		<head>
			<title><xsl:value-of select="@className"/></title>
	...
	<xsl:template match="summary">
		...
		<tr>
		   <td class="sev4">High: <xsl:value-of select="@high"/></td>
		   <td class="sev3">Significant: <xsl:value-of select="@significant"/></td>
		   <td class="sev2"><b>Moderate: <xsl:value-of select="@moderate"/></b></td>
		   <td class="sev1"><b>Low: <xsl:value-of select="@low"/></b></td>
		   <td class="sev0">Info: <xsl:value-of select="@info"/></td>
		</tr>
		</table>
		<font size='-1'>
			<xsl:if test="@lineCoverage">
				Code coverage: <xsl:value-of select="@lineCoverage"/>%&#160;&#160;&#160;
			</xsl:if>
			<xsl:if test="@branchCoverage">
				Branch coverage: <xsl:value-of select="@branchCoverage"/>%&#160;&#160;&#160;
			</xsl:if>
	...
	<xsl:template match="source">
		<table id="sourcecode" class="sourcecode" border="0" cellpadding="0" cellspacing="0" style="width: 100%">
			<tbody>
	...
	<xsl:template match="line/diag">
		...
		<a>
			...
			<xsl:variable name="sev"><xsl:value-of select="/classDetails/diags/diag[@id=$id]/@sev"/></xsl:variable>			
			<xsl:choose>
			...
			<xsl:value-of select="/classDetails/diags/diag[@id=$id]/text()"/></xsl:attribute>
			<xsl:attribute name="href"><xsl:value-of select="concat('#diagList', @id)"/></xsl:attribute>
			<xsl:value-of select="count(//diags/diag[@id=$id]/preceding-sibling::*)+1"/></a>
	</xsl:template>
	...
	<xsl:template match="diags/diag">
		...
		<tr>
			<td>
				...
				<b><a><xsl:attribute name="href"><xsl:value-of select="concat('#diag', @id)"/></xsl:attribute><xsl:value-of select="position()"/></a></b>
			</td>
			<td>
				<a><xsl:attribute name="href"><xsl:value-of select="concat('#diag', @id)"/></xsl:attribute><xsl:value-of select="count(//line/diag[@id=$id]/../preceding-sibling::line)+1"/></a>
			</td>
			<td>
				<xsl:choose>
					<xsl:when test="(@sev = '0')">Info</xsl:when>
			...
			<td>
				<a><xsl:attribute name="href">javascript:popupwindow('<xsl:value-of select="concat(//classDetails/@pathToRoot, 'rules/', @tool, '/', @rule)"/>.html');</xsl:attribute><xsl:value-of select="text()"/></a><br/>
			</td>
		</tr>	
	</xsl:template>
